 ...
 *
 * 绑定（首次，2分钟）：
 *   调 bind_feishu → 自动弹出配置引导 → 点链接创建应用拿到凭证 →
 *   设好环境变量 → 再调 bind_feishu → 浏览器打开 → 飞书扫码授权 → 完成
 *
 *   之后所有 Agent 调 export_feishu 直接导出，token 自动刷新。
 */
...
// ---- 获取有效 token（过期自动刷新）----
async function getAccessToken(dataDir) {
    ...
    const res = await fetch(`${BASE}/authen/v1/refresh_access_token`, {
      ...
      body: JSON.stringify({
        ...
        refresh_token: saved.refresh_token,
         app_id: process.env.FEISHU_APP_ID, app_secret: APP_SECRET
      })
    });
...
export async function bindFeishu(dataDir) {
  return new Promise((resolve, reject) => {
    ...
    const authUrl = `https://open.feishu.cn/open-apis/authen/v1/authorize?app_id=${APP_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&state=feishu-mcp&response_type=code&scope=bitable:app+im:image+doc:document`;
    ...
    const server = createServer(async (req, res) => {
      ...
      if (url.pathname === "/callback") {
        ...
        const tokenRes = await fetch(`${BASE}/authen/v1/access_token`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ grant_type: "authorization_code", code,  app_id: process.env.FEISHU_APP_ID, app_secret: APP_SECRET })
        });
...
function getAuth(dataDir) {
  if (!APP_ID || !APP_SECRET) return { needConfig: true };
  return { dataDir };
}

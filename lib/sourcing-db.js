...
export function exportToExcel(db, outputPath) {
  const results = db.prepare(`
    ...
    LEFT JOIN sourcing_taobao_matches t ON p.best_taobao_id = t.taobao_product_id
    ORDER BY p.profit_rate DESC
  `).all();
  ...
  const headers = [
    ...
    '淘宝最佳价格', '淘宝单价', '利润金额', '利润率',
    '淘宝标题', '淘宝店铺', '发货地', '淘宝链接', '淘宝截图', '创建时间'
  ];

...
export function buildBootstrapGuide() {
  ...
  return [
    ...
    "",
    "1. 克隆或更新 `https://github.com/13739777296-del/ecommerce-sourcing-mcp.git`。",
    "2. 在项目目录运行 `npm ci`。",
    ...
    "## 初始化后",
    "",
    "1. 调用 `ecommerce_sourcing_accounts list` 查看账号池。",
    "2. 没账号就调用 `ecommerce_sourcing_accounts add`。",
    "3. 调用 `ecommerce_sourcing_accounts open_login` 打开本机 Chrome 登录页。",
    "4. 用户登录后调用 `ecommerce_sourcing_accounts check`。"
  ].join("\n");
}

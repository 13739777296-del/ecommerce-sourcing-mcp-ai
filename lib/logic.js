...
const PRODUCT_FAMILY_DEFINITIONS = [
  ...
  { id: "calcium", label: "钙", pattern: /钙片|液体钙|柠檬酸钙|calcium/i },
...
export function assessProfit(input) {
  const profitAmount = roundMoney(input.jdUnitPrice - input.taobaoUnitPrice);
  const profitRate = input.taobaoUnitPrice > 0 ? roundRate(profitAmount / input.taobaoUnitPrice) : 0;
  if (profitRate < 0.35) {
    return { qualified: false, rule: "rate", profitAmount, profitRate, reason: "利润率不足 35%" };
  }
  if (profitRate > 0.60) {
    return { qualified: false, rule: "rate", profitAmount, profitRate, reason: "利润率超过 60%，淘宝价可能异常" };
  }
...
export function buildTaobaoSearchKeyword({ brand, title }) {
  ...
  const normalizedTitle = String(title || "")
    ...
    .replace(/[【】\[\]()（）]/g, " ")
    .replace(/强化装|组合装|囤货装|家庭装|优惠装|买一送一|新老包装随机|官方|旗舰店/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function extractChineseBrandAliases(title = "") {
  ...
  const beforeForm = [...normalized.matchAll(/([\u4e00-\u9fa5]{2,6})(?=软?胶囊|片|颗粒|保健品)/g)]
    ...
    .filter((item) => !/辅酶|还原型|维生素|保健品|健康|活力|心肌|心脏|脑血管/.test(item));
  aliases.push(...beforeForm);
  const branded = normalized.match(/[A-Za-z][A-Za-z0-9+.-]{1,20}([\u4e00-\u9fa5]{2,6})/);
  if (branded?.[1] && !/还原型|辅酶|维生素/.test(branded[1])) aliases.push(branded[1]);
...
export function assessSameProductMatch(jdProduct = {}, taobaoProduct = {}, keyword = "") {
  ...
  const brandBonus = sharedBrand(jdText, taobaoText) ? 0.12 : 0;
...
function extractSearchTokens(keyword = "") {
  ...
  for (const token of ["辅酶q10", "辅酶", "q10", "coq10", "nad", "xikang", "swisse", "斯维诗", "gnc", "普丽普莱", "纽维可", "还原型", "胶囊", "软胶囊", "奶蓟草", "水飞蓟", "护肝", "护旰", "护肝片", "肝护片", "养肝片", "净肝片", "胆碱", "姜黄", "叶黄素", "养肺", "护肺", "润肺", "呼吸", "蜂胶", "葡萄籽", "原花青素", "烟酰胺", "卵磷脂", "软磷脂", "记忆力", "银杏", "圣洁莓", "月经", "经期", "内分泌", "卵巢", "促排卵", "维生素c", "维c", "vc"]) {
...
function productSearchText(product = {}) {
  return [product.title, product.skuText, product.shopName].filter(Boolean).join(" ");
}
...
function assessSkuCompatibility(jdProduct = {}, taobaoProduct = {}) {
  const jdSku = parseSkuProfile(jdProduct.skuText || jdProduct.title || "");
  const taobaoSku = parseSkuProfile(taobaoProduct.skuText || taobaoProduct.title || "");
  ...
  if (ratio > 4) {
    return {
      compatible: false,
      strong: false,
      reason: `SKU 数量差异过大：京东 ${jdSku.contentCount}${jdSku.contentUnit || ""}，淘宝 ${taobaoSku.contentCount}${taobaoSku.contentUnit || ""}`
    };
  }
  return {
    compatible: true,
    strong: ratio <= 1.5,
    reason: `SKU 可比：京东 ${jdSku.contentCount}${jdSku.contentUnit || ""}，淘宝 ${taobaoSku.contentCount}${taobaoSku.contentUnit || ""}`
  };
}
...
function sharedBrand(left = "", right = "") {
  ...
  return brandGroups.some((group) => group.some((token) => normalizedLeft.includes(token)) && group.some((token) => normalizedRight.includes(token)));
}

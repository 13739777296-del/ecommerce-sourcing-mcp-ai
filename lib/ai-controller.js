...
function normalizeAllowedBrandsForJd(allowedBrands) {
  ...
  for (const item of allowedBrands) {
    ...
    if (!brand || key.length < 2 || seen.has(key)) continue;
    seen.add(key);
    result.push(brand);
  }
  return result;
...
export async function aiJdHarvest(page, brand, opts = {}) {
      ...
      for (const target of shopProds) {
        ...
        try {
          ...
          const candidate = {
            ...
            skuInfo: detail.skuInfo || "", brand: detail.brand || target.matchedAllowedBrand || "",
  ...
  return {
    ...
    rejected,
    stats: { totalHarvested: allCandidates.length, passed: qualified.length, rejected: rejected.length, shopsCollected: shopNames.size, shopsTried: shopsToTry.length, allowedBrands: allowedBrands.length, shopStats }
  };
}

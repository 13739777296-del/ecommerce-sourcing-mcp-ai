...
export async function exportToFeishu(db, dataDir) {
  ...
  // 2. 加字段
  const fieldNames = ["京东标题","京东价格","京东店铺","京东截图","淘宝价格","利润金额","利润率","淘宝标题","淘宝店铺","发货地","淘宝截图","京东链接","淘宝链接"];
  for (const f of fieldNames) {
    await fetch(`${BASE}/bitable/v1/apps/${appToken}/tables/${tableId}/fields`, {
      method: "POST", headers: h(), body: JSON.stringify({ field_name: f, type: 1 })
    }).then(r => r.json());
  }
  ...
  for (const r of rows) {
    ...
    if (jdImg) fields["京东截图"] = [{ image_key: jdImg, width: 300, height: 400 }];
    if (tbImg) fields["淘宝截图"] = [{ image_key: tbImg, width: 300, height: 400 }];

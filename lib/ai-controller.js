...
export async function aiJdSearch(page, keyword, opts = {}) {
  ...
  // 6. 提交：优先回车（真人最常用，比找按钮稳）
  console.log(`[AI] 回车提交搜索`);
  ...
  if (!currentUrl.includes('search.jd.com') && productsCount === 0) {
    ...
    throw new Error(`搜索失败，未跳结果页且0商品。当前URL: ${currentUrl}，截图: ${shot}`);
  }
 ...
 * AI滚动页面
 */
export async function aiScrollPage(page, times = 3) {
...
export async function aiExtractShopAllProducts(page, shopName, maxPages = 5) {
  ...
  for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
    ...
    console.log(`[AI] 第${pageNum}页: 该店新增${newProducts.length}个（页内共${products.length}个）`);
...
export async function aiJdHarvest(page, brand, opts = {}) {
  ...
  console.log(`[AI] === 京东选品开始: "${keyword}" (翻${maxPages}页, 取详情≤${maxDetail}, 评价>${minComments}) ===`);

  // 1) 搜索 + 翻页，先把买手店品的"列表信息"攒够
  ...
  for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
    ...
    for (const p of products) {
      if (p.shopType === "buyer" && !seenSku.has(p.productId)) {
        seenSku.add(p.productId);
        buyerList.push(p);
      }
    }
    console.log(`[AI] 第${pageNum}页累计买手店品: ${buyerList.length}`);
    if (pageNum < maxPages) {
      const ok = await aiJdNextPage(page);
      if (!ok) break;
    }
  ...
  console.log(`[AI] 列表阶段完成，买手店品 ${buyerList.length} 个，开始进详情拿评价(最多${maxDetail}个)...`);

  // 2) 逐个进详情页拿评价数（用验证过的"新标签页用完即关、列表页恢复"逻辑）
  const candidates = [];
  let detailDone = 0;
  let passed = 0;

  const limit = Math.min(buyerList.length, maxDetail);
  for (let k = 0; k < limit; k++) {
    const target = buyerList[k];
    // 在当前列表页找到该sku对应的卡片索引（列表可能因翻页变了，按sku重新定位）
    ...
    try {
      if (idx < 0) {
        // 该sku不在当前页（翻页后），直接用列表信息，评价数留空待补
        candidates.push({ ...target, comments: null, fromList: true });
      } else {
        ...
        const pass = commentsNum > minComments;
        if (pass) passed++;

        // 截图存本地（进了详情页就整页截，路径存库）
        let screenshotPath = "";
        if (shotDir) {
          try {
        ...
        candidates.push({
          ...
          price: detail.price || target.price,
      ...
      console.log(`[AI] 第${k + 1}个进详情失败: ${e instanceof Error ? e.message.slice(0, 40) : e}`);
      if (!page.url().includes("search.jd.com")) {
        await page.goto(listUrl, { waitUntil: "domcontentloaded", timeout: 30000 }).catch(() => {});
        await page.waitForTimeout(1500);
      }
    ...
    // 真人间隔
    if (k < limit - 1) await randomPause(4000, 8000);
  }

  // 只保留评价>2的（fromList的没拿到评价，先不计入通过）
  const qualified = candidates.filter((c) => c.passComments);

  console.log(`[AI] === 京东选品完成: 进详情${detailDone}个, 评价>${minComments}的${passed}个 ===`);

  return {
    candidates: qualified,
    stats: {
      buyerListCount: buyerList.length,
      detailDone,
      passedComments: passed
    }
  };
}
...
export async function aiTaobaoHarvest(page, keyword, opts = {}) {
  ...
  console.log(`[AI] === 淘宝选品开始: "${keyword}" (国内:${requireDomestic} 48h:${require48h} 已售≥${minSales}) ===`);
  ...
  console.log(`[AI] 列表${all.length}个 → 筛后${filtered.length}个(国内+48h+已售≥${minSales})`);

  // 3) 符合的品逐个进详情页拿SKU规格（淘宝详情多在新标签页打开）
  const candidates = [];
  let detailDone = 0;
  const limit = Math.min(filtered.length, maxDetail);
  const context = page.context();

  for (let k = 0; k < limit; k++) {
    const target = filtered[k];
    try {
      // 按 productId 在当前列表页定位链接并点击
      const linkLoc = page.locator(`a[href*="id=${target.productId}"]`).first();
      const exists = await linkLoc.count();
      if (!exists) {
        // 列表里找不到（懒加载/翻页），用列表信息兜底，SKU留空
        candidates.push({ ...target, skuInfo: "", detailOk: false });
      } else {
        const pagesBefore = context.pages().length;
        ...
        // 详情页发货地更准，重算 isDomestic（只认"发货地"标签附近的地名，不全文搜）
        const finalShipFrom = detail.shipFrom || "";
        const isOverseas = /(香港|澳门|台湾)/.test(finalShipFrom);
        // 国内 = 标签明确国内省份 或 "中国大陆"标记；发货地没抓到则不判定(交Agent看截图)
        const detailDomestic = detail.mainlandFlag || (!!finalShipFrom && !isOverseas);
        // 只有"明确识别到发货地是港澳台"才剔除；抓不到发货地不剔除(避免误杀店名带"香港"但国内发货的)
        const rejectReason = (requireDomestic && isOverseas)
          ? `发货地确认港澳台(${finalShipFrom})` : "";

        // 截图存本地（进了详情页就整页截，路径存库，清洗交给Agent）
        let screenshotPath = "";
        if (shotDir) {
          try {
            mkdirSync(shotDir, { recursive: true });
        ...
        candidates.push({
          ...
          sales: target.sales,
          salesNum: target.salesNum,
          shipFrom: finalShipFrom,
          isDomestic: detailDomestic,
          ship48h: target.ship48h,
          shipHours: detail.shipHours,
          skuInfo: detail.skuInfo || "",
          skuOptions: detail.skuOptions || [],
          shop: detail.shop || target.shop,
          imgSrc: target.imgSrc || "",
          url: target.url,
          screenshotPath,
        ...
        // 恢复列表页：新标签页关掉回列表；原页跳转重新goto
        if (detailPage !== page) {
          await detailPage.close().catch(() => {});
          await page.bringToFront().catch(() => {});
        } else if (!page.url().includes("s.taobao.com")) {
          await page.goto(listUrl, { waitUntil: "domcontentloaded", timeout: 30000 }).catch(() => {});
          await page.waitForTimeout(1500);
        }
      }
    } catch (e) {
      console.log(`[AI] 淘宝第${k + 1}个进详情失败: ${e instanceof Error ? e.message.slice(0, 40) : e}`);
      if (!page.url().includes("s.taobao.com")) {
        await page.goto(listUrl, { waitUntil: "domcontentloaded", timeout: 30000 }).catch(() => {});
        await page.waitForTimeout(1500);
      }
    }
  ...
  console.log(`[AI] === 淘宝选品完成: 筛后${filtered.length}个, 进详情${detailDone}个 ===`);

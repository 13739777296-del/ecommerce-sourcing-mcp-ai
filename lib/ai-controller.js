...
export async function aiJdSearch(page, keyword) {
  ...
  console.log(`[AI] 已跳转到搜索页`);

  // 等待商品卡片出现（最多等30秒）
  try {
    await page.waitForSelector('div[data-sku]', { timeout: 30000 });
    console.log(`[AI] ✅ 商品卡片已加载`);
  } catch (e) {
    console.log(`[AI] ⚠️  等待超时，尝试继续...`);
  }

  await page.waitForTimeout(3000);  // 再等3秒确保完全加载
  await aiScrollPage(page, 3);

  // 检查结果
  const productsCount = await page.locator('div[data-sku]').count();
  console.log(`[AI] 页面加载完成，找到 ${productsCount} 个商品`);

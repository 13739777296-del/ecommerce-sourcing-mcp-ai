...
export function extractSpec(text) {
  ...
  const patterns = [
    // 数量+单位（优先级从高到低，长单位优先）
    { regex: /(\d+(?:\.\d+)?)\s*(粒|片|袋|包|盒|瓶|罐|支|条|个|枚)(?![a-zA-Z])/,type: 'count' },
    { regex: /(\d+(?:\.\d+)?)\s*(千克|公斤|kg)/i, type: 'weight', unit: 'g', toGrams: x => x * 1000 },  // kg优先
    { regex: /(\d+(?:\.\d+)?)\s*(毫克|mg)/i, type: 'weight', unit: 'g', toGrams: x => x / 1000 },
    { regex: /(\d+(?:\.\d+)?)\s*(克|g)(?![a-zA-Z一-龥])/i, type: 'weight', unit: 'g', toGrams: x => x },  // g最后
    { regex: /(\d+(?:\.\d+)?)\s*(升|l)(?![a-zA-Z一-龥])/i, type: 'volume', unit: 'ml', toMl: x => x * 1000 },  // L优先
    { regex: /(\d+(?:\.\d+)?)\s*(毫升|ml)/i, type: 'volume', unit: 'ml', toMl: x => x },
  ];
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      if (pattern.type === 'count') {
        ...
        const standardValue = pattern.toGrams(value);
        return {
          type: 'weight',
          value,
          unit: 'g',
        ...
        const standardValue = pattern.toMl(value);
        return {
          type: 'volume',
          value,
          unit: 'ml',

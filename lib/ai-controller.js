...
export async function aiJdSearch(page, keyword, opts = {}) {
  ...
  // 1. 优先在当前页直接搜（搜索结果页顶部也有搜索框，像真人改词重搜，不必整页回首页）。
  //    只有当前不在京东任何页、或确实找不到搜索框时，才 goto 首页兜底。
  const alreadyOnJD = page.url().includes('jd.com');
  let existingBox = alreadyOnJD ? await aiFindSearchBox(page).catch(() => null) : null;
  if (existingBox) {
    console.log(`[AI] 已在京东页面，当前页直接改词重搜（不回首页）`);
  } else {
    await page.goto("https://www.jd.com", { waitUntil: "domcontentloaded", timeout: 30000 });
    console.log(`[AI] 不在可搜索的京东页，已回首页`);
  }
  ...
  // 6.5 评论数倒序：京东 PC 端"按评论总数倒序"是前端 JS 排序，不走 URL 参数（psort 已失效，实测确认），
  ...
  if (opts.sortByComments) {
    ...
    if (!sorted) console.log(`[AI] ⚠️ 评论排序点击未生效`);
  }
...
export async function aiJdSortByComments(page) {
  try {
    ...
    if (!(await trigger.isVisible({ timeout: 5000 }).catch(() => false))) {
      console.log("[AI] 未找到排序栏'综合'触发器（页面可能未加载完）");
      return false;
    }
    ...
    if (!(await option.isVisible({ timeout: 3000 }).catch(() => false))) {
      ...
      if (!(await option.isVisible({ timeout: 3000 }).catch(() => false))) {
        console.log("[AI] 下拉里未出现'按评论总数倒序'选项");
        return false;
      }
    }
    ...
    console.log("[AI] ✅ 已切换为按评论总数倒序");
    return true;
  } catch (e) {
    console.log(`[AI] 评论排序点击失败: ${e instanceof Error ? e.message.slice(0, 40) : e}`);
    return false;
  }
}
...
export async function aiJdHarvest(page, brand, opts = {}) {
  ...
  for (const shopName of shopsToTry) {
    ...
    if (searchResult.sorted === false) {
      ...
      console.log(`[日志] 店铺「${shopName}」评论排序未生效，跳过（连续第 ${consecutiveSortFailures} 次）`);

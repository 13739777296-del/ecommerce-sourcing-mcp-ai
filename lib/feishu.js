...
export async function exportToFeishu(db, dataDir) {
  ...
  // 1.5 获取首列名（改名为"京东店铺"，失败则用原名"文本"）
  const fieldsResp = await fetch(`${BASE}/bitable/v1/apps/${appToken}/tables/${tableId}/fields`, { headers: h() });
  const fieldsData = await fieldsResp.json();
  const firstField = fieldsData.data?.items?.[0];
  const firstFieldName = firstField?.field_name || "文本";
  if (firstField && firstFieldName !== "京东店铺") {
    await fetch(`${BASE}/bitable/v1/apps/${appToken}/tables/${tableId}/fields/${firstField.field_id}`, {
      method: "PUT", headers: h(), body: JSON.stringify({ field_name: "京东店铺" })
    }).then(r => r.json()).catch(() => {});
  }

  // 2. 加字段（类型: 1=文本 2=数字 15=超链接 17=附件）
  const fieldDefs = [
    ["京东标题",1],["京东价格",2],["京东截图",17],["淘宝价格",2],["利润金额",2],
    ["利润率",1],["淘宝标题",1],["淘宝店铺",1],["发货地",1],["淘宝截图",17],
    ["京东链接",15],["淘宝链接",15]
  ];
  ...
  // 2.5 读回字段列表确认创建成功
  const actualFields = await fetch(`${BASE}/bitable/v1/apps/${appToken}/tables/${tableId}/fields`, { headers: h() }).then(r => r.json());
  const fieldNames = new Set((actualFields.data?.items || []).map(f => f.field_name));
  console.log("[飞书] 实际字段:", [...fieldNames].slice(0, 6).join(", "), "...");

  // 3. 读数据 + 上传截图到多维表格（用drive API，不是im API）
  const rows = rawDb.prepare(`SELECT p.jd_title, p.jd_price, p.jd_shop, p.jd_url, p.jd_screenshot_path, t2.taobao_price, t2.profit_amount, t2.profit_rate, t2.taobao_title, t2.taobao_shop, t2.taobao_ship_from, t2.taobao_url, t2.taobao_screenshot_path FROM sourcing_products p JOIN sourcing_taobao_matches t2 ON p.jd_product_id = t2.jd_product_id ORDER BY t2.profit_rate DESC LIMIT 200`).all();
  ...
  for (const r of rows) {
    ...
    const fields = {
      ...
      "京东链接": r.jd_url ? { text: "京东", link: r.jd_url } : null,
      "淘宝链接": r.taobao_url ? { text: "淘宝", link: r.taobao_url } : null
    };

    // 只保留表里真实存在的字段（参照 feishu-scenarios-mcp 的校验逻辑）
    const validFields = {};
    for (const [k, v] of Object.entries(fields)) {
      if (fieldNames.has(k)) validFields[k] = v;
    }
    if (Object.keys(validFields).length === 0) continue;

    const res = await fetch(`${BASE}/bitable/v1/apps/${appToken}/tables/${tableId}/records`, {
      method: "POST", headers: h(), body: JSON.stringify({ fields: validFields })
    });
    const data = await res.json();
    if (data.code === 0) count++;
  }

  return { ok: true, count, url: `https://bytedance.feishu.cn/base/${appToken}` };
}

...
export async function fullSelectionFlow(ctx, db, keyword, options = {}) {
  ...
  console.log(`\n[清理] 关闭浏览器会话...`);
  try {
    const { closeAiSession } = await import('./ai-controller.js');
    await closeAiSession('ai-jd');
    await closeAiSession('ai-taobao');
    console.log(`[清理] ✅ 浏览器已关闭`);
  } catch (e) {
    console.log(`[清理] 关闭失败: ${e.message}`);
  }

...
export async function aiJdHarvest(page, brand, opts = {}) {
  ...
  for (const shopName of shopsToTry) {
    ...
    for (let pageNum = 1; pageNum <= maxPagesPerShop; pageNum++) {
      ...
      const products = await aiExtractJdProducts(page, 60);
      const filtered = filterJdProductsForShop(products, shopName, brand, allSeenSku);
      ...
      console.log(`[日志] 页${pageNum} 店铺「${shopName}」原始${products.length}个，品牌轻筛命中${shopProds.length}个，跳过：${formatSkipStats(filtered.skipped)}`);
      ...
      if (pageNum < maxPagesPerShop && qualifiedCount < targetCount) {
        const ok = await aiJdNextPage(page);
        if (!ok) break;
      }
    }
  ...
  return {
    ...
    rejected,
    stats: { totalHarvested: allCandidates.length, passed: qualified.length, rejected: rejected.length, shopsCollected: shopNames.size, shopsTried: shopsToTry.length, shopStats }
  };
}
...
function filterJdProductsForShop(products, shopName, brand, seenSkuSet) {
  const skipped = {
    noProductId: 0,
    duplicate: 0,
    shopMismatch: 0,
  ...
  for (const product of products || []) {
    ...
    if (!jdProductMatchesBrandSeed(product, brand)) {
      skipped.brandMismatch += 1;
      if (examples.length < 5) examples.push(`非品牌:${title || product.productId}`);
      continue;
    }

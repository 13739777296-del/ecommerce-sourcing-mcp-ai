...
export function buildTaobaoSearchKeyword({ brand, title }) {
  ...
  // 词典没覆盖到品类时的兜底：抽取一个简短干净的"核心品名"，绝不把整条长标题塞进去。
  const core = extractCoreProductName(title, normalizedBrand);
  return [normalizedBrand, core].filter(Boolean).join(" ").replace(/\s+/g, " ").trim();
}

// 通用核心品名提取（不依赖品类词典）：去掉品牌、产地、营销词、规格/数字/英文，
// 只保留前若干个有意义的中文词，输出短关键词。供 fallback 用。
function extractCoreProductName(title = "", brand = "") {
  let text = String(title || "");
  // 去品牌（英文整体 + 括号内英文 + 品牌里的中文片段）
  for (const part of String(brand || "").split(/[\s（）()\/、,，]+/).filter(Boolean)) {
    text = text.split(part).join(" ");
  }
  text = text
    .replace(/[A-Za-z0-9+.\-/]+/g, " ")                 // 去所有英文和数字（含 magnesium、90/180 等）
    .replace(/[【】\[\]()（）:：*※·,，;；、]/g, " ")
    .replace(/\d+\s*(?:mg|g|kg|ml|l|毫克|克|千克|毫升|升|粒|片|颗|瓶|盒|罐|袋|件)/gi, " ")
    // 产地/营销/通用修饰词，去掉
    .replace(/原装|进口|正品|现货即发|现货|包邮|官方|旗舰店|海外|跨境|保税|强化装|组合装|囤货装|家庭装|优惠装|买一送一|新老包装随机|增强免疫|提高代谢|高倍|焕活|成人|中老年|儿童|婴幼儿|男士|女士|韩国|日本|美国|英国|德国|法国|澳洲|澳大利亚|加拿大|新西兰|香港/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  // 取前 6 个汉字左右的核心词（按空格切，最多保留 2 段、合计不超过 8 字）
  const segs = text.split(/\s+/).filter((s) => /[一-龥]/.test(s));
  let core = "";
  for (const s of segs) {
...
export function extractBrand(title = "") {
  // \u652f\u6301\u591a\u8bcd\u82f1\u6587\u54c1\u724c\uff08\u5982 "GOOD HEALTH"\u3001"VITA GROW"\uff09\uff0c\u6700\u591a\u53d6\u8fde\u7eed\u4e24\u4e2a\u82f1\u6587\u8bcd\uff0c\u907f\u514d\u622a\u6210 "GOOD"\u3002
  const twoWord = String(title).match(/[A-Za-z][A-Za-z0-9+.-]{1,20}(?:\s+[A-Za-z][A-Za-z0-9+.-]{1,20})?/);
  return twoWord?.[0]?.trim() ?? String(title).match(/^[\u4e00-\u9fa5]{2,6}/)?.[0] ?? "";
}
...
// 通用关键词相关性安全网：从关键词里抽出"有意义的词"（英文/数字串 + 中文词段），
// 标题命中其中任意一个才算相关。用来拦掉淘宝搜索返回的完全无关品（如搜"硫辛酸"返回"铁架床"），
// 不依赖硬编码品类表——任意品类都适用。过于泛的词(胶囊/片/丸等)不作为判据，避免误放。
const GENERIC_KEYWORD_TOKENS = new Set([
  "胶囊", "软胶囊", "片剂", "营养", "正品", "进口", "原装", "保健", "补充剂",
  "capsule", "tablet", "softgel", "mg", "ml"
]);
...
export function keywordTokens(keyword = "") {
  ...
  for (const m of raw.matchAll(/[a-z0-9]{2,}/g)) tokens.add(m[0]);
  for (const m of raw.matchAll(/[一-鿿]{2,}/g)) tokens.add(m[0]);
  return [...tokens].filter((token) => !GENERIC_KEYWORD_TOKENS.has(token));
}

export function keywordRelevant(title = "", keyword = "") {
  const tokens = keywordTokens(keyword);
  if (tokens.length === 0) return true; // 抽不出可判据的词就不拦，交给后续人工/视觉复核
  const normalizedTitle = String(title).toLowerCase();
  return tokens.some((token) => normalizedTitle.includes(token));
}

// 严格相关性：标题必须包含搜索词里的【每一个】关键 token 才算相关（用户要求：
// 只有包含搜索词所有字的才点进详情，否则不看）。比 keywordRelevant(任一命中)严得多，
// 大幅减少进详情次数=省风控+省时间。抽不出 token 时不拦（交给后续人工/视觉复核）。
export function keywordAllTokensMatch(title = "", keyword = "") {
  const tokens = keywordTokens(keyword);
...
// 含每个字（用户要求）：标题必须包含搜索词里的【每一个有效字符】（中文字 + 英文/数字），
// 位置可不连续。比按词(token)更宽松——只要这些字都散落在标题里就算相关。
// 忽略空格和分隔符；纯通用词(胶囊等)和噪声不参与（用 keywordTokens 已过滤的字符集）。
export function keywordAllCharsMatch(title = "", keyword = "") {
  const normalizedTitle = String(title).toLowerCase();
  const raw = String(keyword).toLowerCase();
  // 中文按单字拆；连续英文/数字按整段(不拆成单字母，否则"abc"会被拆成a/b/c几乎必中)。
  const chars = new Set();
  for (const m of raw.matchAll(/[一-鿿]/g)) chars.add(m[0]);
  for (const m of raw.matchAll(/[a-z0-9]{2,}/g)) chars.add(m[0]);
  if (chars.size === 0) return true;
  return [...chars].every((c) => normalizedTitle.includes(c));
}

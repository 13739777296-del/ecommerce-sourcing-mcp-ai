...
export async function aiJdHarvest(page, brand, opts = {}) {
  ...
  for (const shopName of shopsToTry) {
    ...
    for (let pageNum = 1; pageNum <= maxPagesPerShop; pageNum++) {
      ...
      const shopProds = products.filter((p) =>
        p.shop && p.shop.includes(shopName) && !allSeenSku.has(p.productId) && jdProductMatchesBrandSeed(p, brand)
      );
      console.log(`[日志] 页${pageNum} 该店${shopProds.length}个（原始${products.length}个，已按品牌"${brand}"轻筛）`);
  ...
  return {
    ...
    rejected,
    stats: { totalHarvested: allCandidates.length, passed: qualified.length, rejected: rejected.length, shopsCollected: shopNames.size, shopsTried: shopsToTry.length }
  };
}
...
export async function aiTaobaoHarvest(page, keyword, opts = {}) {
  ...
  return { candidates, rejected, stats: { detailDone, passed: candidates.length, rejected: rejected.length } };
}

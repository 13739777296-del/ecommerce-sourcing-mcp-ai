 ...
 * AI滚动页面
 */
export async function aiScrollPage(page, times = 1) {
...
export async function aiJdHarvest(page, brand, opts = {}) {
  ...
  const maxPagesPerShop = opts.maxPagesPerShop || 3;
  const minComments = opts.minComments ?? 2;
  const shotDir = opts.screenshotDir || "";
  const searchKeyword = `${brand} 买手店`;

  console.log(`[AI] === 京东选品: "${searchKeyword}" (目标${targetCount}品) ===`);
  ...
  console.log(`[AI] 收集到 ${shopNames.size} 个买手店: ${[...shopNames].slice(0,5).join(', ')}...`);

  // 2) 逐个搜买手店名 → 进详情
  for (const shopName of shopNames) {
    if (allCandidates.length >= targetCount) break;
    console.log(`[AI] --- 扒店: "${shopName}" ---`);
    ...
    for (let pageNum = 1; pageNum <= maxPagesPerShop; pageNum++) {
      ...
      console.log(`[AI] 第${pageNum}页: 该店${shopProds.length}个`);
      ...
      for (const target of shopProds) {
        ...
        try {
          ...
          allCandidates.push({
            ...
            shopUrl: detail.shopUrl || "", comments: detail.comments || "0",
            commentsNum, skuInfo: detail.skuInfo || "", brand: detail.brand || "",
            imgSrc: target.imgSrc || "", url: detail.url || target.url,
            screenshotPath, passComments: pass
          });
          console.log(`[AI]   ${allCandidates.length}/${targetCount} ${(detail.title||'').slice(0,20)} 评${commentsNum} ${pass?'✓':'✗'}`);
  ...
  console.log(`[AI] === 京东选品完成: 共拉${allCandidates.length}个, 评价>2的${qualified.length}个 ===`);
  return {
...
export async function aiExtractTaobaoDetail(page) {
  ...
  const detail = await page.evaluate(() => {
    ...
    if (!shop) {
      // 兜底：seller-info里的name
      const el = document.querySelector('[class*="seller-info"] [class*="name"], [class*="ShopInfo"] [class*="name"]');
      if (el && el.innerText) shop = el.innerText.trim();
    }
    if (!shop) {
      // 最后兜底：找含"店"字且长度合理的链接
      const links = Array.from(document.querySelectorAll('a, span'));
      const l = links.find(x => { const t=(x.innerText||'').trim(); return t.length>=3&&t.length<25&&/店$/.test(t)&&!/免费|我要|入驻|淘宝/.test(t); });
      if (l) shop = l.innerText.trim();
    }

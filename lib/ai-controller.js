...
/**
 * 京东搜索（AI驱动）
 * 策略：先模拟人类操作搜索框（防风控），如果跳转失败就直接构造URL
 */
export async function aiJdSearch(page, keyword) {
  ...
  try {
    ...
    if (!currentUrl.includes('www.jd.com') || currentUrl.includes('search.jd.com')) {
      console.log(`[AI] 跳转回首页...`);
    ...
    if (searchBox) {
      ...
      await randomPause(100, 200);
      await aiTypeText(page, keyword);
      await randomPause(800, 1200);  // 多等一会，确保输入完成
      ...
      if (btnVisible) {
        ...
        // 5. 等待跳转
        await page.waitForURL(/search\.jd\.com/, { timeout: 8000 }).catch(() => {});

        // 6. 检查是否成功
        if (page.url().includes('search.jd.com')) {
          console.log(`[AI] ✅ 搜索成功（点击按钮）`);
    ...
    console.log(`[AI] 策略1失败: ${e.message}`);
  }

  // 策略2：直接构造URL跳转（兜底，100%成功）
  console.log(`[AI] 使用策略2：直接构造URL`);
  const encodedKeyword = encodeURIComponent(keyword);
  const searchUrl = `https://search.jd.com/Search?keyword=${encodedKeyword}&enc=utf-8`;

  await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
  await randomPause(2000, 3000);
  await aiScrollPage(page, 3);

  console.log(`[AI] ✅ 搜索成功（URL跳转）`);
  return {
    url: page.url(),

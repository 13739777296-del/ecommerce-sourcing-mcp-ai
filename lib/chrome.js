...
export function normalizeChromeSessionOptions(options = {}) {
  return {
    ...
    terminateConflictingProfile: options.terminateConflictingProfile !== false
  };
}
...
export async function closeManagedChromeSessions(profileDir = "") {
  ...
  for (const [key, state] of entries) {
    ...
    await state.browser.newBrowserCDPSession().then((session) => session.send("Browser.close")).catch(() => undefined);
    await state.browser.close().catch(() => undefined);
    await waitForChromeProfileExit(key).catch(() => undefined);
    closed += 1;
  }
  return closed;
}
...
function createChromeSessionHandle(profileDir, state, sessionOptions) {
  return {
    ...
    close: async () => {
      ...
      await state.browser.newBrowserCDPSession().then((session) => session.send("Browser.close")).catch(() => undefined);
      await state.browser.close().catch(() => undefined);
      // 兜底：CDP Browser.close 对 persistent profile 有时不退进程，按 profile 精确杀干净
      await waitForChromeProfileExit(profileDir).catch(() => undefined);
    }
  };
}

...
export async function aiTaobaoSearchByImage(page, imageUrl) {
  ...
  // 1. 跳转淘宝拍立淘（以图搜图）
  await page.goto("https://s.taobao.com/search?imgfile=&js=1&stats_click=search_radio_all%3A1&initiative_id=staobaoz_20110221&ie=utf8&tfsid=&app=imgsearch",
    { waitUntil: "domcontentloaded", timeout: 30000 });
  await randomPause(2000, 3000);

  // 2. 找图片上传区域（淘宝有多种方式：粘贴URL、上传文件、拖拽）
  // 最简单的方式：直接在URL里带imgfile参数重新跳转
  const searchUrl = `https://s.taobao.com/search?imgfile=${encodeURIComponent(imageUrl)}&commend=all&ssid=s5-e&search_type=item&sourceId=tb.index&initiative_id=tbindexz_20170306&ie=utf8&tfsid=&app=imgsearch`;

  await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
  console.log(`[AI] 淘宝以图搜图已触发`);
...
export async function aiTaobaoSearch(page, keyword) {
  console.log(`[AI] 淘宝关键词搜索: ${keyword}`);
...
export async function aiExtractTaobaoProducts(page, maxCount = 10) {
  ...
  const products = await page.evaluate((max) => {
    // 淘宝的商品卡片（多种可能的结构）
    const cardSelectors = [
      'div[data-category="auctions"]',  // 搜索结果页
      '.item',
      '.Card--mainPicAndDesc',
      'div[class*="item"]'
    ];
    ...
    if (cards.length === 0) {
      ...
      cards = Array.from(links)
        ...
        .map(a => a.closest('div[class*="item"]') || a.closest('div[class*="card"]') || a.closest('div') || a.parentElement);
    }
    ...
    for (let i = 0; i < cards.length && results.length < max; i++) {
      ...
      let title = '';
      for (const line of lines) {
        if (line.length > 8 && !/^¥|^\d+\.\d+$|^￥/.test(line) && !/付款|月销|人付款|包邮|发货/.test(line)) {
          title = line;
          break;
        }
      }
      ...
      // 发货地（关键字段：国内发货 vs 海外）
      let shipFrom = '';
      const shipFromMatch = text.match(/([一-龥]{2,6})\s*发货|发货地[:：]\s*([一-龥]{2,6})/);
      if (shipFromMatch) {
        shipFrom = shipFromMatch[1] || shipFromMatch[2];
      }
      // 判断是否国内
      const isDomestic = shipFrom && !/(香港|澳门|台湾|海外|国外|保税)/.test(shipFrom);

      // 发货时效（关键字段：48小时内发货）
      let shipHours = null;
      const shipTimeMatch = text.match(/(\d+)\s*小时.*发货|(\d+)\s*天.*发货/);
      if (shipTimeMatch) {
        shipHours = shipTimeMatch[1] ? parseInt(shipTimeMatch[1]) : parseInt(shipTimeMatch[2]) * 24;
      }
      ...
      // 图片
      const img = card.querySelector('img');
      const imgSrc = img ? (img.src || img.getAttribute('data-src') || img.getAttribute('data-img') || '') : '';
